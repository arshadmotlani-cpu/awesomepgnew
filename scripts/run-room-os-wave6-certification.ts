#!/usr/bin/env npx tsx
/**
 * Room OS Wave 6 certification — Workflow + Business Metrics parity.
 * Read-only — never modifies production data.
 *
 *   npm run cert:room-os-wave6
 *   npm run cert:room-os-wave6:json
 */
  ...
  if (!process.env.DATABASE_URL?.trim()) {
    console.error(
      'DATABASE_URL is not set. Run:\n' +
        '  npx vercel env run --environment production npm run cert:room-os-wave6',
    );
    process.exit(1);
  }
