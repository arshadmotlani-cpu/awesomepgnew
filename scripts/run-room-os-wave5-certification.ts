#!/usr/bin/env npx tsx
/**
 * Room OS Wave 5 certification — Shantinagar parity + rules DB + timeline Layer B.
 * Read-only — never modifies production data.
 *
 *   npm run cert:room-os-wave5
 *   npm run cert:room-os-wave5:json
 *
  ...
  if (!process.env.DATABASE_URL?.trim()) {
    console.error(
      'DATABASE_URL is not set. Run:\n' +
        '  npx vercel env run --environment production npm run cert:room-os-wave5',
    );
    process.exit(1);
  }
