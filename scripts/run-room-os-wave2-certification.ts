#!/usr/bin/env npx tsx
/**
 * Room OS Wave 2 certification — Shantinagar parity release gate.
 * Read-only — never modifies production data.
 *
 *   npm run cert:room-os-wave2
 *   npm run cert:room-os-wave2:json
 *
 * Exit 1 only when report.status === 'fail' (warnings allowed pre-cutover).
 */
