...
import { getAdvancePaymentReceiptPreviewAction } from '@/src/hair/actions/advancePayment';
...
type PreviewData = Extract<
  Awaited<ReturnType<typeof getAdvancePaymentReceiptPreviewAction>>,
  { ok: true }
>;
...
export function AdvancePaymentReceiptViewer({ invoiceId, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      const res = await getAdvancePaymentReceiptPreviewAction(invoiceId);
      if (cancelled) return;
      if (!res.ok) {
        setPreview(null);
        setError(res.error);
  ...
  const shareWhatsApp = () => {
    ...
    const body = `Hi ${preview.customerName}, your advance payment receipt ${preview.invoiceNumber} for ${preview.grandTotalLabel} is ready: ${publicUrl}`;
  ...
  const viewer = (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Advance payment receipt"
      data-testid="advance-payment-receipt-viewer"
    >
        ...
        <div className="min-w-0 flex-1">
          ...
          {preview ? (
            <p className="qs-invoice-viewer-header-meta">
              <span>Receipt: {preview.invoiceNumber}</span>
              <span className="qs-invoice-viewer-meta-sep">·</span>
              <span>{preview.grandTotalLabel}</span>
            </p>
          ) : pending ? (
      ...
      <div className="qs-invoice-viewer-toolbar" aria-label="Receipt actions">
        {preview ? (
        ...
        <div className="qs-invoice-viewer-sheet-wrap">
          ...
          {error ? (
            <div className="qs-invoice-viewer-error">
              <p>{error}</p>
              <p>Advance was saved. Open the receipt for full details.</p>
              <Link href={`/billing/${invoiceId}`}>
