...
export async function releaseQuickSaleHoldForCheckout(
  ...
  const { nextInvoiceNumberForTx, applyPaidSideEffects } = await import(
  ...
  const [existing] = await hairDb
    ...
    .where(
      and(
        eq(fyhInvoices.id, holdInvoiceId),
        eq(fyhInvoices.source, 'quick_sale'),
        eq(fyhInvoices.status, 'draft'),
        eq(fyhInvoices.customerId, input.customerId),
      ),
  ...
  await hairDb.transaction(async (tx) => {
    const invoiceNumber = await nextInvoiceNumberForTx(tx as unknown as typeof hairDb);
    await tx.delete(fyhInvoiceLines).where(eq(fyhInvoiceLines.invoiceId, holdInvoiceId));
    ...
    await tx
      ...
      .where(eq(fyhInvoices.id, holdInvoiceId));
    ...
    if (grandTotalPaise === 0) {
      await applyPaidSideEffects(tx as unknown as typeof hairDb, holdInvoiceId);
    }
    ...
    await tx.insert(fyhCustomerTimeline).values({
      ...
      metadata: { invoiceId: holdInvoiceId, source: 'quick_sale' },
    });
  });

  return holdInvoiceId;
}
