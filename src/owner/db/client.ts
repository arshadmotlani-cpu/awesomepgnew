...
import { getOwnerDatabaseUrl, assertOwnerDatabaseIsolated } from '@/src/owner/lib/db/env';
import * as schema from '@/src/owner/db/schema';

type DrizzleClient = ReturnType<typeof drizzle<typeof schema>>;

type DbGlobal = { sql?: Sql; drizzle?: DrizzleClient };
const GLOBAL_KEY = '__ownerDb' as const;
...
function init(): DrizzleClient {
  ...
  assertOwnerDatabaseIsolated();
  const { connectionString, options } = parseDatabaseUrl(url);
  const sql = postgres(connectionString, {
    ...options,
    connection: { application_name: `owner-${process.env.NODE_ENV ?? 'development'}` },
  });
...
export const ownerDb: DrizzleClient = new Proxy({} as DrizzleClient, {
  get(_target, prop, receiver) {
    return Reflect.get(init() as object, prop, receiver);
  },
});

export function createOwnerClient(options?: { max?: number }) {
  const url = getOwnerDatabaseUrl();
  const parsed = parseDatabaseUrl(url);
  const client = postgres(parsed.connectionString, {
    ...parsed.options,
    max: options?.max ?? 1,
    connection: { application_name: 'owner-script' },
  });
  return {
