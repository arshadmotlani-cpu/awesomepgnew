...
import { getOwnerDatabaseHost } from '@/src/owner/lib/db/env';

const MIGRATIONS_FOLDER = 'src/owner/db/migrations';
const MIGRATIONS_SCHEMA = 'drizzle_owner';
const MIGRATIONS_TABLE = '__drizzle_migrations';

async function main() {
  const host = getOwnerDatabaseHost();
  console.log(`Owner OS DB host: ${host ?? 'unknown'}`);
  console.log(`→ Running Owner OS migrations from ${MIGRATIONS_FOLDER} …`);
  ...
  console.log(
    applied > 0 ? `✓ Applied ${applied} Owner OS migration(s)` : '✓ Owner OS migrations up to date',
  );
  await close();
}
