...
const GLOBAL_KEY = '__platformDb' as const;
...
function init(): DrizzleClient {
  ...
  const sql = postgres(connectionString, {
    ...options,
    connection: { application_name: `platform-${process.env.NODE_ENV ?? 'development'}` },
  });
...
export const platformDb: DrizzleClient = new Proxy({} as DrizzleClient, {
  get(_target, prop, receiver) {
    return Reflect.get(init() as object, prop, receiver);
  },
});

export function createPlatformClient(options?: { max?: number }) {
  const url = getPlatformDatabaseUrl();
  const parsed = parseDatabaseUrl(url);
  const client = postgres(parsed.connectionString, {
    ...parsed.options,
    max: options?.max ?? 1,
    connection: { application_name: 'platform-script' },
  });
  return {
