...
export function ElectricityMeterNotice({ variant = 'banner' }: { variant?: Variant }) {
  ...
  return (
    <div className="rounded-xl border border-amber-500/30 bg-amber-950/40 px-4 py-3 text-sm text-amber-100">
      <p className="font-semibold">AC electricity billing</p>
      <p className="mt-1 text-amber-200/90">
        General electricity is included in rent. AC usage is billed separately per tenant, based on
        meter readings. Missing meter photos will result in system-generated average billing.
      </p>
    </div>
  );
}
