...
const LISTED_PGS =
  'Central PG, Central PG (Female), and Trimurti Nagar';
...
export function MarkCentralOccupiedButton() {
  ...
  async function onClick() {
    const confirmed = window.confirm(
      `Mark ${LISTED_PGS} as fully occupied? Public site will show no beds available.`,
    );
    if (!confirmed) return;
    setPending(true);
    setError(null);
  ...
  return (
    <div className="rounded-xl border border-amber-500/30 bg-amber-950/20 p-4">
      <p className="text-sm font-medium text-amber-100">Full occupancy (listed PGs)</p>
      <p className="mt-1 text-xs text-amber-100/80">
        Mark {LISTED_PGS} as fully occupied — no beds available on /pgs.
      </p>
      <div className="mt-3 flex flex-wrap items-center gap-3">
        ...
        >
          {pending ? 'Updating…' : 'Mark listed PGs fully occupied'}
        </button>
