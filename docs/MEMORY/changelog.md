...
- **NOTICE-RENT-COVERAGE** — Notice deduction excludes days already covered by paid rent (charge window = last N days before vacate); SSOT `noticeDeductionEngine.ts` + `noticeDeduction.ts`; migration `0120_notice_rent_coverage.sql`; shared `NoticeDeductionBreakdown` UI on admin/resident move-out surfaces; migrate script + verify script updated; 1321 tests green
