...
- **Room OS / Property OS (Wave 1 in progress)** — Wave 0 shipped (`src/roomOs/`, outbox, rules catalog v1, projector skeleton). **Now:** Wave 1 Occupancy Engine + Bed Brain live-read (`buildBedBrainSnapshot`, `room-os/v1/loadBed`). **Still blocked for Ops Centre UI:** wireframes approval. Dhruv incident **closed** — no further work.
