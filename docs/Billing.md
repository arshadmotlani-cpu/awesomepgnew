...
**Deposit refund unlock (2026-06-23):** `depositRefundUnlock.ts` computes resident refund eligibility (locked until 11 AM IST checkout for fixed stays; after vacate date for monthly). Deduction preview uses `computeNoticeDeduction()` from `billing.ts` (missing notice days × daily rent). PG-level `average_electricity_bill_paise` supports checkout average fallback.
