...
| **Harish — stale refund** | `listPendingDepositRefunds` and `refund_pending` action items ignored completed checkout with `final_refund_paise = 0`. `admin_deposit_refund_status` could remain `pending` after zero-refund settlement. | Exclude terminal zero-refund settlements from ops refund lists; resolve stale `refund_pending` / unresolved checkout actions. |
